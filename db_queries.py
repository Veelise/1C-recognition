...
# 1.2. Получить эскизы сотрудника
GET_SKETCHES_BY_EMPLOYEE = """
...
# 1.4. Получить первичные чертежи сотрудника
...
# 2.2 Связать PRO с PRIMARY_DRAWINGS
UPDATE_PRO_WITH_DRAWING = """
UPDATE PRO
SET idPrimaryDrawing = %s
WHERE idPrmRes = %s;
"""
# Параметры: (id_первичного_чертежа, id_результата_pro)
...
# 4.2 Обновить статус валидации в PRO
UPDATE_PRO_VALIDATED = """
UPDATE PRO
SET validated = TRUE, validated_by = %s, validation_date = %s
WHERE idPrmRes = %s;
"""
# Параметры: (id_валидатора, дата_валидации, id_pro)

# 4.3 Получить FRO по PRO
GET_FRO_BY_PRO = """
...
INSERT INTO ARCH_OF_DRAWS (
    FilePathPrmArch, AssociatedWithPrmArch, NumDravFROArch,
    NameDravFROArch, DesignationFROArch, ProjectCodeFROArch,
    DevFROArch, DateOriginalCreationFROArch, OriginalPaperFormatFROArch,
    NumberOfSheetsFROArch, SaveDateArch, MonthCheckArch,
    SFilePathArch, DateAddedArch, NumNameArch,
    UploadedEdmployeeIDArch, id_employeePrmArch
)
VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
RETURNING idPrmArch;
"""
# Параметры: (путь_к_файлу, связь, номер_чертежа, наименование, обозначение,
#             код_проекта, разработчик, дата_создания, формат, кол-во_листов,
#             дата_архивации, месячная_проверка, путь_к_эскизу, дата_добавления,
#             название, id_загрузившего, id_сотрудника)
...
GET_ALL_ARCHIVES = """
SELECT idPrmArch, FilePathPrmArch, AssociatedWithPrmArch, NumDravFROArch,
       NameDravFROArch, DesignationFROArch, ProjectCodeFROArch,
       DevFROArch, DateOriginalCreationFROArch, OriginalPaperFormatFROArch,
       NumberOfSheetsFROArch, SaveDateArch, MonthCheckArch,
       SFilePathArch, DateAddedArch, NumNameArch,
...
SELECT
    e.idEmployee,
    e.FullName,
    COUNT(DISTINCT pd.id) AS primary_drawings_count,
    COUNT(DISTINCT a.idPrmArch) AS archived_count
FROM EMPLOYEES e
